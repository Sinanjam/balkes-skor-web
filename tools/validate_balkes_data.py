...
def main():
    ...
    for season in manifest.get("availableSeasons", []) or []:
        sid = season.get("id")
        if not sid:
            errors.append("availableSeasons item without id")
            continue
        ...
        for rel in ["season.json", "matches_index.json", "standings_by_week.json"]:
            path = sdir / rel
            if not path.exists():
                errors.append(f"Missing {sid}/{rel}")
            else:
                must_json(path)
        ...
        if not isinstance(idx, list):
            errors.append(f"{idx_path} is not a list")
            idx = []

        for m in idx:
            mid = str(m.get("id", ""))
            if not mid:
                errors.append(f"{idx_path}: match without id")
                continue
            if mid in detail_ids:
                errors.append(f"Duplicate match id: {mid}")
            detail_ids.add(mid)

            detail_url = m.get("detailUrl") or f"seasons/{sid}/matches/{mid}.json"
            detail_path = data_dir / detail_url
            if not detail_path.exists():
                errors.append(f"Missing detail file: {detail_url}")
                continue
            detail = must_json(detail_path)
            if detail and str(detail.get("id")) != mid:
                errors.append(f"Detail id mismatch: {detail_url}")

    report = must_json(data_dir / "data_report.json") if (data_dir / "data_report.json").exists() else {}
    if isinstance(report, dict):
        total_report = report.get("totalAppMatches")
        if total_report is not None and int(total_report) != total_manifest:
            errors.append(f"data_report.totalAppMatches={total_report} but manifest total={total_manifest}")

    md = []
    md.append("# Balkes data validation")
    md.append("")
    md.append(f"- Manifest seasons: `{len(manifest.get('availableSeasons', []) or [])}`")
    md.append(f"- Manifest total matches: `{total_manifest}`")
    md.append(f"- Detail IDs: `{len(detail_ids)}`")
